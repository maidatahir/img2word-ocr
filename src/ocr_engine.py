...
def preprocessImage(imagePath):
    ...
    binaryImg = cv2.adaptiveThreshold(grayImg, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, blockSize=31, C=15)
...
def detectAlignment(bbox, pageWidth):
    x1, _, x2, _ = bbox
    cX = (x1 + x2) / 2
    relPos = cX / pageWidth
...
def runOcr(imagePath, localMode=True):
    ...
    if not wordsList:
        ...
        for line in rawText.splitlines():
            ...
            if cLine:
                pList.append({"text": cLine, "isHeading": isTitleLine(line) or isTitleLine(cLine), "alignment": "LEFT", "runs": [{"text": cLine, "bold": False, "italic": False}]})
