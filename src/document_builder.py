...
def applyParagraphFormat(para, alignValue):
...
def addHeading(doc, headingText, headingLevel=1):
...
def addBodyParagraph(doc, block):
...
def buildDocx(ocrResult, outputPath):
    doc = Document()
...
def buildPdf(ocrResult, outputPath):
...
def buildTxt(ocrResult, outputPath):
    rawText = ocrResult.get("rawText", "")
    if not rawText:
        lines = [p.get("text", "") for p in ocrResult.get("paragraphs", [])]
        rawText = "\n".join(lines)
